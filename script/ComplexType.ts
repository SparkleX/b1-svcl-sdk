...
const outputFolder = "src/complexType";
...
const source = fs.readFileSync(`${__dirname}/ComplexType.handlebars`, "utf8");
const template = handlebars.compile(source);
fs.mkdirSync(outputFolder, { recursive: true });

const schema = jsonData["edmx:Edmx"]["edmx:DataServices"].Schema
for (const item of schema.ComplexType) {
    if(item.Name == "Table2TypeMapping" ) {
        continue;
    }
